...
## 2026-06-23 迭代式推理检索 P0/P1 执行记录

- [x] 新增迭代式树检索 prompt，要求 LLM 返回 `selected_nodes`、`sufficiency`、`missing_information`、`next_search_focus`。
- [x] 单文档 LLM 检索从单轮节点选择升级为最多 3 轮：读取节点、判断是否充分、不足则继续。
- [x] `debug_json` 保存 `retrieval_rounds`，逐轮记录选中节点、充分性判断、缺失信息和下一轮焦点。
- [x] 知识库级聚合新增 `document_retrieval_rounds`，保留每个文档的检索轮次。
- [x] 增加最小交叉引用识别：`详见`、`参见`、`附录`、`表`、`图`、`第...章`。
- [x] 交叉引用目标可匹配 PageIndex 树节点，并作为下一轮 `交叉引用候选`。
- [x] 评测集新增 5 条 `iterative_reasoning` 样例，覆盖信息抽取、风险判断、定位和交叉引用。

### 迭代式检索验收标准

- 证据不足时，检索轮次应继续，而不是直接生成模糊答案。
- 证据充分时，检索应提前停止，避免无意义扩大证据。
- debug 中必须能看到每轮选了哪些节点、为什么选、是否充分、还缺什么。
- 明确交叉引用应进入下一轮候选，但只匹配标题和摘要，不做全文泛化扩展。
