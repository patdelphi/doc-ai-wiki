...
| 实体归一与查询扩展 | 已通过 | 已新增最小实体词表和 `query_normalizer`，入库 FTS、全文检索、AI 质检检索查询复用同一套归一扩展逻辑 |
| 正式质量指标 | 部分通过 | 已新增 50 条检索样例、50 条 Claim 样例、50 条 PageIndex 问题、规则样例和固定指标计算；本地只读检索评测已运行但标准证据 ID 尚未对齐 |
...
- `python -m pytest tests/unit/test_query_normalizer.py tests/unit/test_retrieval.py tests/unit/test_quality.py -q`：25 passed
- `python -m pytest tests/unit/test_ingest_quality.py tests/integration/test_app.py::test_register_document_and_query_status_should_work tests/integration/test_app.py::test_vector_and_hybrid_search_should_return_results_after_ingest -q`：8 passed
- `python -m pytest tests/unit/test_retrieval_evaluation.py tests/unit/test_evaluation_fixtures.py -q`：11 passed
- `python -m pytest tests/unit/test_evaluation_fixtures.py -q`：4 passed
...
- PageIndex 仍需基于真实本地知识库运行真实 LLM 与离线降级分组评测结果。
- 实体词表仍是最小样例，后续需要结合 50 条检索/Claim 评测集持续扩展。
- 50 条检索样例仍需与真实本地知识库的 `doc_uid` / `chunk_id` 对齐。
