...
def format_evidence_detail_html(evidence: dict | None) -> str:
    ...
    metadata_html = _build_panel_html(
        ...
        cards=[
            ...
            ("定位", _display_text(resolved.get("source_span"))),
            ("标题路径", _display_text(resolved.get("heading_path"))),
            ("来源锚点", _display_text(resolved.get("source_anchor"))),
            ("Chunk 类型", _display_text(resolved.get("chunk_type"))),
            ("证据关系", relation_label),
            ("检索来源", _display_text(resolved.get("retrieval_source"))),
...
def format_evidence_detail_markdown(evidence: dict | None) -> str:
    ...
    return "\n".join(
        [
            ...
            f'- 定位：{_display_text(resolved.get("source_span"))}',
            f'- 标题路径：{_display_text(resolved.get("heading_path"))}',
            f'- 来源锚点：{_display_text(resolved.get("source_anchor"))}',
            f'- Chunk 类型：{_display_text(resolved.get("chunk_type"))}',
            f'- 内容 Hash：{_display_text(resolved.get("content_hash"))}',
