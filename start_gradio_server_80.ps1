# Program description: Start the FastAPI backend and Gradio frontend for server access with automatic port fallback.
...
$frontendPort = Get-FreePort -StartPort 80 -HostValue $hostValue
...
Write-Host "Backend URL: http://$displayHost`:$backendPort"
Write-Host "Frontend URL: http://$displayHost`:$frontendPort"
